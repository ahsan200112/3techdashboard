...
const EditProductPage = () => {
  ...
  const { getCategoriesList, getProductById, getProductOptions, updateProduct } = productService();
  ...
  const handleOptionSelect = (index, selected) => {
    ...
    setVariantSelectedOptions(prev => ({
      ...
      [index]: {
        ...(prev[index] || {}),
        [selected.value._id]: '' // empty for now, value will be selected below
      }
    }));
    setOptionDropdownVisibility(prev => ({ ...prev, [index]: false }));
  };
  ...
  const handleRemoveOption = (index, optionId) => {
    ...
    // Optional: re-open dropdown after removal
  ...
  // Load product data, categories and product options when component mounts
  useEffect(() => {
  ...
  const setVariantSelectedOptionsFromData = (variants) => {
    const newSelectedOptions = {};

    variants.forEach((variant, index) => {
      newSelectedOptions[index] = {};

      if (!variant.valueIds) return;

      variant.valueIds.forEach((valueObj) => {
        const optionId = valueObj.productOptionId?._id;
        const valueId = valueObj._id;

        if (optionId && valueId) {
          newSelectedOptions[index][optionId] = valueId;
        }
      });
    });

    setVariantSelectedOptions(newSelectedOptions);
  };
  ...
  const loadProduct = async () => {
    try {
      ...
      reset({
        ...
        discount: {
          disocuntType: product.discount?.disocuntType || 'percentage',
          disocuntValue: product.discount?.disocuntValue || 0,
        ...
        variants: product.variants?.map(v => ({ ...v, _id: v._id })) || []
      });

      // Set images if available
      if (product.images && product.images.length > 0) {
        const formattedImages = product.images.map(img => ({
          id: img._id || Date.now() + Math.random(),
          _id: img._id, // ✅ include _id explicitly
          preview: img.image,
          isMain: img.isMain,
          existingImage: true
        }));
      ...
      setVariantSelectedOptionsFromData(product.variants);
  ...
  const removeImage = (imageId) => {
    setImages((prev) => {
      ...
      // ✅ If image is from DB, store its _id for deletion
      if (imageToRemove?.existingImage && imageToRemove?._id) {
      ...
      // ✅ Ensure there's still a main image
    ...
    try {
      ...
      images.forEach((image, index) => {
        if (image.file) {
          ...
          formData.append(`existingImages[${index}]`, image.preview);
          formData.append(`existingImagesIsMain[${index}]`, image.isMain);
        }
      });
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                {images.map((image) => (
                  <div key={image.id} className="relative group">
                    <div
                      className={`w-full h-32 rounded-lg border-2 ${image.isMain ? 'border-blue-500' : 'border-slate-200'} overflow-hidden`}
                    >
                      <img
                        src={image.preview}
                        alt="Product"
                    ...
                    <div className="absolute top-2 left-2 flex items-center">
                      ...
                      />
                      <span className="ml-1 text-xs bg-white/80 px-1 rounded text-slate-700">Main</span>
                    </div>
                  </div>
                ))}

                {/* ✅ Upload Button (up to 10 images) */}
                {images.length < 10 && (
                    ...
                    <div key={field.id} className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                        ...
                        <div>
                          ...
                          <input
                            type="hidden"
                            {...register(`variants.${index}.valueIds`)} // already correct
                          />
                        </div>
                      </div>
            ...
            <div className="bg-white rounded-2xl p-6 border border-slate-200">
              <div className="flex items-center justify-between mb-4">
                ...
                <label className="flex items-center cursor-pointer relative">
                  ...
                  <div className="w-9 h-5 bg-slate-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 rounded-full relative transition-colors duration-300 peer-checked:bg-blue-600">
                    <div className="absolute top-[2px] left-[2px] bg-white border border-slate-300 rounded-full h-3 w-3 transition-all duration-300 peer-checked:translate-x-4 peer-checked:border-white"></div>
                  </div>
                </label>
              </div>
                ...
                <div className="space-y-3 animate-fade-in">
                  <div>
                    ...
                    {errors.discount?.disocuntType && (
                      <p className="text-red-500 text-xs mt-1">{errors.discount.disocuntType.message}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {errors.discount?.disocuntValue && (
                      <p className="text-red-500 text-xs mt-1">{errors.discount.disocuntValue.message}</p>
                    )}
                  </div>
                  ...
                  {discountedPrice > 0 && (
                    <div className="p-3 bg-green-50 border border-green-100 rounded-lg">
                      <div className="flex items-center text-green-700">
                        ...
                        <span className="text-sm font-medium">Discounted Price: ${discountedPrice}</span>
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      {errors.discount?.startDate && (
                        <p className="text-red-500 text-xs mt-1">{errors.discount.startDate.message}</p>
                      )}
                    </div>
                    <div>
                      ...
                      {errors.discount?.endDate && (
                        <p className="text-red-500 text-xs mt-1">{errors.discount.endDate.message}</p>
                      )}
                    </div>
                  </div>
                </div>
